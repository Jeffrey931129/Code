...
// event queue -> {timestamp, event type: 0 for departure while 1 for arrival}
set<pair<int, int>> event_queue; // early -> late then departure -> arrival 
...
bool assignTable(int time) {
    ...
    event_queue.insert({time+guest.duration, 0});
...
int main() {
    ...
    event_queue.insert({arr[gid].arrival, 1}); // generate the first guest arrival
    while (event_queue.size()) {
        ...
        switch (type) {
            ...
            case 1: // arrival
                ...
                assignTable(arr[gid].arrival);
                if(gid<n-1) event_queue.insert({arr[++gid].arrival, 1}); // generate the next guest arrival
                break;
        }

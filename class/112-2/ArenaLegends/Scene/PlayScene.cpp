...
static Engine::Rectangle* white;
static Engine::Label* genshin;
...
void PlayScene::Initialize() {
    ...
    if (!onlineMode) {
        ...
        B_CardSelect.insert(5);
        B_CardSelect.insert(3);
    }
    
    // test
    AddNewObject(turtle = new Engine::Sprite("loading/1.jpg", halfW, halfH, 26*77, 26*54, 0.5, 0.5));
    turtle->Tint = al_map_rgba(255, 255, 255, 0);
    AddNewObject(white = new Engine::Rectangle(0, 0, 1800, 1400, al_map_rgba(255, 255, 255, 0)));
    AddNewObject(genshin = new Engine::Label("Full Score", "recharge.otf", 200, halfW, halfH, 0, 0, 0, 0, 0.5, 0.5));
}
...
void PlayScene::Update(float deltaTime) {
    ...
    if (tick > 500.5 && turtlePicture < (int)((tick-500.5)*30+1)) {
        ...
        // if (tick < 502.6) white->color = al_map_rgba(255, 255, 255, std::min((int)((tick-500.5)*255/2), 255));
        // else if (tick < 504.6) genshin->Color = al_map_rgba(0, 0, 0, std::min((int)((tick-502.5)*255/2), 255));
        // white->Update(deltaTime); 
        // genshin->Update(deltaTime);
    }
    ...
    gameData.B.elixir += deltaTime * gameData.elixirSpeed;          // for singal
    if (gameData.B.elixir > 10 && !onlineMode) {
        gameData.B.elixir = 10;
        if (gameTime < 120) B_WaitForDeployed.push({0, Engine::Point(26, 8)});
    }
            ...
            if (selectedCard->cardType == ARMY) {
                ...
                if (!onlineMode) {                                              // for single
                    ...
                    if ((B_CardSelect.find(singleModeID) != B_CardSelect.end() ||
                        B_CardSelect.size() < 8) && singleModeID != -1) {
                            B_CardSelect.insert(singleModeID);
                            if (singleModeID >= 9) deployAccordingID(singleModeID, nowBlock.x, nowBlock.y, gameTime - 2);
...
void PlayScene::showWinAnimation() {
    ...
    // bgmInstance = AudioHelper::PlaySample("genshin.ogg", false, AudioHelper::BGMVolume);
}

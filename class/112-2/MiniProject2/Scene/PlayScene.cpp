...
#include "Turret/Turrent4.hpp"
#include "Turret/Turrent5.hpp"
	...
	for (int i = 0; i < SpeedMult; i++) {
		...
		if (enemyWaveData.empty()) {
			if (EnemyGroup->GetObjects().empty()) {
				...
				// 這些delete有問題，會導致win crash，跟delete順序有關
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	// 			if (turret && turret->Position.x == x * BlockSize + BlockSize / 2 && turret->Position.y == y * BlockSize + BlockSize / 2) {
	// 				EarnMoney(turret->GetPrice() / 2);
	// 				TowerGroup->RemoveObject(obj->GetObjectIterator());
	// 				mapState[y][x] = TILE_FLOOR;
	// 				AudioHelper::PlayAudio("shovel.mp3");
	// 				break;
	// 			}
	// 		}
	// 	}
	...
	// 	UIGroup->RemoveObject(preview->GetObjectIterator()); preview = nullptr;
	// 	return;
	// }
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode == ALLEGRO_KEY_TAB) {
		...
		if (keyCode == ALLEGRO_KEY_ENTER && keyStrokes.size() == code.size()) {
			...
			EffectGroup->AddNewObject(new Plane());			// plane
			EarnMoney(10000);
		}
	}
...
void PlayScene::ConstructUI() {
	...
	AddNewObject(new Engine::Label("Q", "pirulen.ttf", 24, 1294, 136, 255, 0, 0, 255, 0.5, 0.5));	// 優化
	// Button 2
	btn = new TurretButton("play/floor.png", "play/dirt.png",
	...
	UIGroup->AddNewControlObject(btn);
	AddNewObject(new Engine::Label("W", "pirulen.ttf", 24, 1370, 136, 255, 0, 0, 255, 0.5, 0.5));
	// Button 3
	btn = new TurretButton("play/floor.png", "play/dirt.png",
	...
	UIGroup->AddNewControlObject(btn);
	AddNewObject(new Engine::Label("E", "pirulen.ttf", 24, 1446, 136, 255, 0, 0, 255, 0.5, 0.5));
	...
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		...
		, 1294, 212, Turrent4::Price);
	btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 3));
	UIGroup->AddNewControlObject(btn);
	AddNewObject(new Engine::Label("R", "pirulen.ttf", 24, 1294, 212, 255, 0, 0, 255, 0.5, 0.5));	// 優化
	// Button5
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/tower-base.png", 1370, 212, 0, 0, 0, 0),
		Engine::Sprite("play/turret-7.png", 1370, 212 - 8, 0, 0, 0, 0)
		, 1370, 212, Turrent5::Price);
	btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 4));
	UIGroup->AddNewControlObject(btn);
	AddNewObject(new Engine::Label("T", "pirulen.ttf", 24, 1370, 212, 255, 0, 0, 255, 0.5, 0.5));	// 優化
	// Button6
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/tower-base.png", 1446, 212, 0, 0, 0, 0),
		Engine::Sprite("play/shovel.png", 1446, 212 - 8, 64, 64, 0, 0)
		, 1446, 212, 0);
	btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 5));
	UIGroup->AddNewControlObject(btn);
	AddNewObject(new Engine::Label("Y", "pirulen.ttf", 24, 1446, 212, 255, 0, 0, 255, 0.5, 0.5));	// 優化
...
void PlayScene::UIBtnClicked(int id) {
	...
	else if (id == 3 && money >= Turrent4::Price)
		preview = new Turrent4(0, 0);
	else if (id == 4 && money >= Turrent5::Price)
		preview = new Turrent5(0, 0);
	else if (id == 5){
		AudioHelper::PlayAudio("shovel.mp3"); shovel_clicked=true;
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
		...
		if(p.y-1>=0&&p.y-1<MapHeight&&p.x>=0&&p.x<MapWidth&&map[p.y-1][p.x]==-1&&mapState[p.y-1][p.x]==TILE_DIRT){
		...
		if(p.y>=0&&p.y<MapHeight&&p.x-1>=0&&p.x-1<MapWidth&&map[p.y][p.x-1]==-1&&mapState[p.y][p.x-1]==TILE_DIRT){

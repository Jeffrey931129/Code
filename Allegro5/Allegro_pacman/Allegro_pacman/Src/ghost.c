...
Ghost* ghost_create(int flag) {
	...
	// TODO-GC-ghost: Create other type ghost, load corresponding sprites.  這邊不確定他們的走法
...
void ghost_destory(Ghost* ghost) {
	// TODO-GC-memory: free ghost resource 完成
...
void ghost_draw(Ghost* ghost) {
	...
	int offset = 0;
	if (al_get_timer_count(move_timer) % 2) {  // 哈哈好用多了
		offset = 0;
	}
	else {
		offset = 16;  
	...
	if (mode_pick==2&&ghost->typeFlag==Clyde&&ghost->status==FREEDOM&& 
		  al_get_timer_count(move_timer) % 120 >= 105 ) {               // 隱鬼
		return;
	}
	...
	*/
	...
	if (ghost->status == FLEE) {
		// TODO-PB-animation: ghost FLEE animation, draw blue flee sprites,  完成
		...
		*/ 
			...
			if (al_get_timer_count(power_up_timer)>=7)
			{
				...
				if (al_get_timer_count(move_timer) % 2) {
					bitmap_x_offset = 32;
				}
			...
			else 
			{
				...
				al_draw_scaled_bitmap(ghost->flee_sprite, offset, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0
				);
		...
		// TODO-PB-animation: ghost going animation  完成
		...
		// TODO-GC-animation: ghost animation  完成
...
void ghost_toggle_FLEE(Ghost* ghost, bool setFLEE) {
	// TODO-PB: change ghosts state when power bean is eaten by pacman.  根本不需要

...
static void ghost_move_script_FREEDOM_random(Ghost* ghost, Map* M) {
	...
	*/
	...
	// TODO-GC-random_movement: (Not in Hackathon)  完成
	// Description:
	...
	switch(ghost->objData.preMove) {
		case RIGHT:
			counter_one = LEFT; break;
		case LEFT:
			counter_one = RIGHT; break;
		case UP:
			counter_one = DOWN; break;
		case DOWN:
			counter_one = UP; break;
		default:
			break;
	}
...
static void ghost_move_script_FREEDOM_shortest_path(Ghost* ghost, Map* M, Pacman* pman)
{
	...
	if (ghost->objData.nextTryMove != counter_one)
		return;
	static Directions proba[4]; // possible movement
	int cnt = 0;
	for (Directions i = 1; i <= 4; i++)
...
static void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	// TODO-PB: escape from pacman   鬼移動很呆
	// Description:
	...
	int cnt = 0;
	for (Directions i = 1; i <= 4; i++)
		if (i != shortestDirection && ghost_movable(ghost, M, i, true)&&i!= shortestDirection)
			proba[cnt++] = i;
	if (cnt >= 1) {
		ghost_NextMove(ghost, proba[generateRandomNumber(0, cnt - 1)]);
	}
...
void ghost_move_script_random(Ghost* ghost, Map* M, Pacman* pacman) {
	if (!movetime(ghost->speed)||ghost->status==STOP)
		return;
		switch (ghost->status)
		{
		case BLOCKED:

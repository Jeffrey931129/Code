...
int main() {
    ...
    for (int i=0; i<n; i++) {
        ...
        while (table_use.size() && (*table_use.begin()).first <= arr[i].arrival-1) {
            int release_time = (*table_use.begin()).first;
            releaseTable(release_time);
        ...
        // clear and assign table on i-th arrival
        if (table_use.size() && (*table_use.begin()).first == arr[i].arrival) {
            releaseTable(arr[i].arrival);
        }
        // attempt to assign table
        while (assignTable(arr[i].arrival));

    }

    // clear all the remaining table
    if (waiting_arrival.size()) {
        while (table_use.size() && (*table_use.begin()).first <= 1e9) {
            int release_time = (*table_use.begin()).first;
            releaseTable(release_time);
            // attempt to assign table
            while (assignTable(release_time));

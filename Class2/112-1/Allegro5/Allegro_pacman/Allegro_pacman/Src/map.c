...
extern ALLEGRO_BITMAP* bomb_picture;
...
Map* create_map(const char * filepath) {
	...
	if (filepath == NULL) {
		...
		// TODO-GC-read_txt: use fopen to open a file stream  完成
	...
	for (int i = 0; i < M->row_num; i++) {
		for (int j = 0; j < M->col_num; j++) {
			if (filepath == NULL)
				...
				// TODO-GC-read_txt: input the map from file to M->map[row][col] 完成
				// '#' -> wall
				// '.' -> beans
				// 'B' -> room of ghost
				// 'P' -> Power Bean 
...
void delete_map(Map* M) {
	if (!M)
		return;
	// TODO-GC-memory: free the dynamic allocated part of Map* M at here;  完成
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				case '#':
					...
					break;
				// TODO-PB: draw the power bean  完成
				
				case 'P':
					draw_power_bean(M,row,col);
					break;
				case 'X':
					al_draw_scaled_bitmap(bomb_picture, 0, 0, 1920, 1788,
